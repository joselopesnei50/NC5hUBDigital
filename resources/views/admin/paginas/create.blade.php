    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-8 max-w-3xl">
        <form action="{{ route('admin.paginas.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    <label class="block text-sm font-bold text-[#0A1128] mb-2">Título da Página (Ex: Quem Somos)</label>
                    <input type="text" name="titulo" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-[#0A1128] focus:ring-[#0A1128]">
                </div>

                <div>
                    <label class="block text-sm font-bold text-[#0A1128] mb-2">Conteúdo HTML/Texto</label>
                    <textarea name="conteudo" rows="10" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-[#0A1128] focus:ring-[#0A1128]"></textarea>
                </div>
            ...
            <div class="mt-8 flex items-center justify-end gap-4">
                <a href="{{ route('admin.paginas.index') }}" class="text-[#8A8F9C] hover:text-[#0A1128] font-bold text-sm transition-colors">Cancelar</a>
